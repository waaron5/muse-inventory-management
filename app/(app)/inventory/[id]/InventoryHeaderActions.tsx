...
import { activateInventoryItem, retireInventoryItem } from "../actions";
...
export function InventoryHeaderActions({
  ...
  async function handleDelete() {
    if (
      !confirm(
        `Delete "${itemTitle}"? It will be retired and moved to the bottom of inventory.`
      )
    ) {
      return;
    }
    ...
    setLoading(true);
    try {
      await retireInventoryItem(itemId);
      toast("Item deleted", "success", {
        duration: 10000,
        actionLabel: "Undo",
        onAction: async () => {
          await activateInventoryItem(itemId);
          router.refresh();
          toast("Item restored");
        },
      });
