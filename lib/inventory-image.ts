...
export const INVENTORY_IMAGE_ACCEPT_ATTRIBUTE = INVENTORY_IMAGE_ACCEPTED_MIME_TYPES.join(",");
...
export function validateInventoryImageFile(file: Pick<File, "size" | "type">) {
  if (
    !INVENTORY_IMAGE_ACCEPTED_MIME_TYPES.includes(
      file.type as (typeof INVENTORY_IMAGE_ACCEPTED_MIME_TYPES)[number],
    )
  ) {
